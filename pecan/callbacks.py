...
from pecan.vne import compute_von_neumann_entropy, find_knee_point
...
class Callback(ABC):
    ...
    @abstractmethod
    def __call__(self, t, X, P, D, A = None):
...
class CalculateLawvereHomology(Callback):
    """Persistent homology calculation callback.

    This class calculates persistent homology based directly on the diffusion matrix of
    each condensation timestep. Since the diffusion matrix is not in general symmetric,
    its associated weighted directed graph does not define a metric space.
    However, the matrix does represent a Lawvere (aka quasipseudometric space).
    We will have to come up with various symmetrizations of this quasipseudometric 
    in order to compute homology.

    """

    def __init__(self, dimension=1, max_cardinality=512, diffusion_time="VNElbow"):
        """Build new instance of callback and set parameters.
    ...
    def __call__(self, t, X, P, D, A):
        ...
        # The Ripser class will return persistence pairs *and* a
        # set of persistence points, i.e. coordinates/distances.
        if self.diffusion_time == "VNElbow":
            entropies = compute_von_neumann_entropy(A)
            time = find_knee_point(entropies)
            A = np.power(A, time)
        else:
            A = np.power(A, t)
            

        A_comp = np.ones_like(A) - A
        
        tuples, points = Ripser(dimension=self.dimension)(A_comp)
    ...
    def finalise(self, data):
        ...
        data.update(
            {
                f"lawvere_persistence_pairs_t_{i}": pairs
                for i, pairs in self.persistence_pairs.items()
            }
        )
        ...
        data.update(
            {
                f"lawvere_persistence_points_t_{i}": pairs
                for i, pairs in self.persistence_points.items()
            }
        )
